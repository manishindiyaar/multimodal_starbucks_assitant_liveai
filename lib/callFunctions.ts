...
export async function startCall(callbacks: CallCallbacks, callConfig: CallConfig, showDebugMessages?: boolean): Promise<void> {
  try {
    ...
    // Start up our Ultravox Session
    uvSession = new UltravoxSession({
      // onStatusChange: (status: any) => callbacks.onStatusChange(status),
      // onTranscriptChange: (transcripts: any) => callbacks.onTranscriptChange(transcripts),
      // onDebugMessage: (msg: any) => callbacks.onDebugMessage?.(msg)
  });
